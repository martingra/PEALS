...
namespace Detector.scr.Image_Processing
{
    public class Segmentacion
    {
        ...
        public Bitmap iniciarWeb(Bitmap imagenBmp, bool esCara)
        {
        ...
        public Image<Gray, byte> iniciar(Image<Bgr, byte> imagen, bool esCara, out Rectangle cmt, out Image<Gray, byte> monitorClasificador, out Image<Gray, byte> imgMogColoreada, bool clasificar)
        {
            ...
            Image<Gray, byte> imgObjetivo = new Image<Gray, byte>(img.Size);
            Image<Gray, byte> rst = new Image<Gray, byte>(img.Size);
            imgMogColoreada = new Image<Gray, byte>(img.Size);
            ...
            else
            {
                ...
                else
                {
                    ...
                    bool checkWidth = (cmt.X > 0 && cmt.X + cmt.Width < img.Size.Width -1);
                    bool checkHeight = (cmt.Y > 0 && cmt.Y + cmt.Height < img.Size.Height - 1);

                    if (checkWidth && checkHeight)
                        imgCara = detectarCara(img.Copy(cmt));
                    else
                        imgCara = new Image<Gray, byte>(cmt.Size);
