...
namespace Detector.scr.Image_Processing
{
    class FaceSegmentacion
    {
        ...
        public Image<Gray, byte> ProcesarCara(Image<Bgr, byte> img_roi, PointF nariz)
        {
            ...
            if (imagenProcesada != null)
            {
                ...
                using (MemStorage storage = new MemStorage())
                {
                    ...
                    if (contour != null && contour.Area > 300)
                    {
        ...
        private Image<Gray, byte> PreprocesarImagen(Image<Bgr, byte> img_roi)
        {
            ...
            try
            {
                imagenProcesada = aplicarOpMorph(imagenProcesada.ThresholdToZero(new Gray(250))).Resize(skin.Width, skin.Height, Emgu.CV.CvEnum.INTER.CV_INTER_NN);
                imagenProcesada = imagenProcesada.Mul(skin);

                if (image_prev != null)
                {
                    img_smooth = img_smooth.Resize(image_prev.Width, image_prev.Height, Emgu.CV.CvEnum.INTER.CV_INTER_NN);
